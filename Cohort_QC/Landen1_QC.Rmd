...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/PGCED_SE_Landen_AN_Sample1
...
png("Landen1_Summary.png", width=18, height=18, units='in', bg="white", res=150,
type=c("cairo"))
par(mfrow=c(2,2))
...
# Remove Samples with 100% missingness
```{bash}
awk '{if ($5 != 1) print $1, $2}' Summary.smiss  | sed -e '1d' > keep_samples.txt
```
...
plink2 --bfile $DATA --keep keep_samples.txt --make-bed --out ${DATA}_cleaned1
```
...
#!/bin/bash
#
#SBATCH --job-name=summary2
#SBATCH --mem=2GB
#SBATCH --nodes=1
#SBATCH --time=0:30:00

DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/PGCED_SE_Landen_AN_Sample1_cleaned1

plink2 --bfile $DATA --missing  --out Summary2
plink2 --bfile $DATA --freq     --out Summary2
plink2 --bfile $DATA --hardy    --out Summary2
```
...
vmiss <- read.table("Summary2.vmiss", col.names = c("CHROM", "ID", "MISSING_CT", "OBS_CT", "F_MISS"))
smiss <- read.table("Summary2.smiss", col.names=c("FID", "IID", "MISSING_CT", "OBS_CT", "F_MISS"))
freq <- read.table("Summary2.afreq", col.names=c("CHROM", "ID", "REF", "ALT", "ALT_FREQS", "OBS_CT"))
hwe <- read.table("Summary2.hardy", col.names=c("CHROM", "ID", "A1", "AX", "HOM_A1_CT", "HET_A1_CT", "TWO_AX_CT", "O(HET_A1)", "E(HET_A1)", "P"))

png("Landen1_Summary2.png", width=18, height=18, units='in', bg="white", res=150,
type=c("cairo"))
par(mfrow=c(2,2))
hist(1-vmiss$F_MISS, breaks="sturges",main="SNPs",col="tan", xlab="Genotyping Rate", ylab="Number of SNPs", labels=T, cex.lab = 2, cex.axis =2, cex.main =2)
hist(1-smiss$F_MISS, breaks="sturges", main="Individuals", col="tan", xlab="Genotyping Rate", ylab="Number of Individuals", cex.lab = 2, cex.axis =2, cex.main =2)
...
plink2 --bfile ${DATA}_cleaned1 --geno 0.1 --make-bed --out ${DATA}_cleaned2

```

grep removed *_cleaned2.log | grep missing

2948 variants removed due to missing genotype data.


## Remove SNPS that violate HEW
...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/PGCED_SE_Landen_AN_Sample1
QC=/nfs/home/aliwal/ANGI/data/QC/Landen1

plink2 --bfile ${DATA}_cleaned2 --hwe 0.000001 --make-bed --out ${DATA}_cleaned3
```

grep removed *_cleaned3.log | grep hwe

--hwe: 1021 variants removed due to Hardy-Weinberg exact test (founders only).
...
plink2 --bfile ${DATA}_cleaned3 --maf 0.01  --chr 1-23 --make-bed --out ${DATA}_cleaned4
```

grep removed *_cleaned4.log | grep ' allele frequency threshold'
118535 variants removed due to allele frequency threshold(s)


## First add sex to the .fam file
```{r}
landen <- read.csv("/nfs/home/aliwal/ANGI/data/sample_sheets/ANGI_Landen-1_samples.txt", sep = "\t")
landen <-landen[,c("chip_well_barcode", "reported_gender")]

fam <- read.table("/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/PGCED_SE_Landen_AN_Sample1_cleaned4.fam", col.names = c("FID", "IID", "X", "X2", "Sex", "Phenotype"))
fam$Sex <- landen$reported_gender[match(fam$IID, landen$chip_well_barcode)]
fam$Sex <- as.vector(ifelse(fam$Sex == "F", 2, ifelse(fam$Sex == "M", 1, 0)))
write.table(fam, "/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/PGCED_SE_Landen_AN_Sample1_cleaned_wsex.fam", col.names = F, row.names = F, sep = "\t", quote=F)
```

```{bash}
cp PGCED_SE_Landen_AN_Sample1_cleaned4.bed PGCED_SE_Landen_AN_Sample1_cleaned_wsex.bed
cp PGCED_SE_Landen_AN_Sample1_cleaned4.bim PGCED_SE_Landen_AN_Sample1_cleaned_wsex.bim
```
...
```{bash}
cat PGCED_SE_Landen_AN_Sample1_noHH.hh | awk -v OFS='\t' '{print $3}'  | uniq > unique_HH.txt
```
...
grep -v OK SexCheck.sexcheck
  ...
  201148270082_R08C01   201148270082_R08C01            2            0      PROBLEM       0.5955


# 9. Remove problem samples
problems.txt:
                FID                   IID
201148270082_R08C01   201148270082_R08C01
...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/PGCED_SE_Landen_AN_Sample1
remove=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/problems.txt
...
png("Landen1_het_F.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/PGCED_SE_Landen_AN_Sample1
path2out=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1
plink --bfile ${DATA}_flipped \
          --freq \
          --out ${path2out}/freq_Landen1
```
...
```{bash}
library(data.table)
dat = data.frame(fread("/scratch/90days/uqawal15/1000G_phase3/Landen1/freq_Landen1.frq", header = T, showProgress = F))
...
## plot
library(ggplot2)
png("Landen1_1KG_AF.png", width=15, height=15, units='in', bg="white", res=150, type=c("cairo"))
...
refout='/scratch/90days/uqawal15/1000G_phase3/Landen1/1000G_phase3_20130502_combined_snpsonly'
...
#SBATCH --time=0:30:00
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/PGCED_SE_Landen_AN_Sample1
pcref='/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/1000G_phase3_20130502_combined_snpsonly'
...
Rscript  /nfs/home/aliwal/software/PCA_ancestrycalling_meanSD.R  LANDEN1    ${pcref}.05.non.outliers.pca3   ${DATA}_cleaned.pca.proj.eigenvec    ${STR}

```

# Table of Samples in each Ancestry
```{r}
anc <- read.table("LANDEN1_ancestry_assigned_with_meanSD_method.txt", header =T)
```
...
```{bash}
cat LANDEN1_ancestry_assigned_with_meanSD_method.txt | sed -e '1d' | awk -v OFS='\t' '{if ($7 == "EUR") print $1, $2}' > EUR_samples.txt
```
...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/PGCED_SE_Landen_AN_Sample1
eur=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1/PCA/EUR_samples.txt
...
```{r}
prefix="PGCED_SE_Landen_AN_Sample1_EUR"
...
png("Landen1_EUR_GRM.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
hist(GRM$off, breaks = seq(-0.1, 1, 0.05), labels = T, main="Distribution of GRM lower triangle", xlab="relationship", cex.main=2, cex.lab=2, cex.axis=2)
dev.off()

off <- GRM$off
off2 <- off[off<0.05]
png("Landen1_EUR_GRM_rm05.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
hist(off2, labels = T,  breaks = seq(-0.1, 0.06, 0.05), main="Distribution of GRM lower triangle < 0.05", xlab="relationship", cex.main=2, cex.lab=2, cex.axis=2)
dev.off()


off3 <- off[off>=0.05]
png("Landen1_EUR_GRM_greater_05.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
hist(off3, labels = T,  breaks = seq(0.05, 1, 0.05), main="Distribution of GRM lower triangle >= 0.05", xlab="relationship", cex.main=2, cex.lab=2, cex.axis=2)
dev.off()
```
...
anc <- read.table(paste0("/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_Landen_AN_Sample1", "/PCA/LANDEN1_ancestry_assigned_with_meanSD_method.txt"), header = T)
anc <- anc %>% select(FID, population)

final <- merge(het, anc, by = "FID")

fam_05 <- read.table("PGCED_SE_Landen_AN_Sample1_EUR_relatedness_005.family.txt", header = F)
related_05 <- unique(fam_05$V1, fam_05$V3)
fam_2 <- read.table("PGCED_SE_Landen_AN_Sample1_EUR_relatedness_02.family.txt", header = F)
...
mv PGCED_SE_Landen_AN_Sample1_final* final
mv PGCED_SE_Landen_AN_Sample1_noHH.hh final
```
