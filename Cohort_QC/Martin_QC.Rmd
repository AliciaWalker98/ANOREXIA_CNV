...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515/PGCED_SE_ANGI_Martin_v1_3_190515
...
png("Martin_Summary.png", width=18, height=18, units='in', bg="white", res=150, type=c("cairo"))
par(mfrow=c(2,2))
...
--geno: 3189 variants removed due to missing genotype data.

# 4.  Remove SNPS that violate HEW
...
--hwe: 1986 variants removed due to Hardy-Weinberg exact test (founders only).
...
114519 variants removed due to allele frequency threshold(s)
...
```{r}
library(dplyr)
martin.s <- read.csv("/nfs/home/aliwal/ANGI/data/sample_sheets/sample_angi_martin_20190429.tsv", sep = "\t")
martin.s <-martin.s[,c("entity.sample_id", "autocall_gender","collaborator_participant_id")]
martin.p <- read.csv("/nfs/home/aliwal/ANGI/data/sample_sheets/participant_angi_martin_20190429.tsv", sep = "\t")
martin <- merge(martin.s, martin.p, by = "collaborator_participant_id")
martin <- martin %>% select(entity.sample_id, entity.participant_id, autocall_gender, reported_gender)
martin$batch <- "Martin"
martin$Sample_ID <-paste0(sapply(strsplit(as.character(martin$entity.sample_id), "_"), "[[", 1), "_",sapply(strsplit(as.character(martin$entity.sample_id), "_"), "[[", 2))
write.table(martin, "/nfs/home/aliwal/ANGI/data/sample_sheets/Martin_Samples.csv", col.names = T, row.names = F, sep = ",", quote=F)

fam <- read.table("/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515/PGCED_SE_ANGI_Martin_v1_3_190515_cleaned4.fam", col.names = c("FID", "IID", "X", "X2", "Sex", "Phenotype"))
fam$Sex <- martin$reported_gender[match(fam$IID, martin$Sample_ID)]
fam$Sex <- as.vector(ifelse(fam$Sex == "F", 2, 1))
write.table(fam, "/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515/PGCED_SE_ANGI_Martin_v1_3_190515_cleaned_wsex.fam", col.names = F, row.names = F, sep = "\t", quote=F)
```

```{bash}
cp PGCED_SE_ANGI_Martin_v1_3_190515_cleaned4.bed PGCED_SE_ANGI_Martin_v1_3_190515_cleaned_wsex.bed
cp PGCED_SE_ANGI_Martin_v1_3_190515_cleaned4.bim PGCED_SE_ANGI_Martin_v1_3_190515_cleaned_wsex.bim
```
...
```{bash}
cat PGCED_SE_ANGI_Martin_v1_3_190515_noHH.hh | awk -v OFS='\t' '{print $3}'  | uniq > unique_HH.txt
```
...
grep -v OK SexCheck.sexcheck
  ...
  201035700167_R02C02   201035700167_R02C02            2            0      PROBLEM        0.474
  201096140126_R09C02   201096140126_R09C02            2            0      PROBLEM        0.769
  201035690089_R02C02   201035690089_R02C02            2            0      PROBLEM       0.2032
  201035690189_R01C02   201035690189_R01C02            2            0      PROBLEM       0.3921


Remove 
201035700167_R02C02   201035700167_R02C02
201096140126_R09C02   201096140126_R09C02
201035690189_R01C02   201035690189_R01C02

# 10. Remove the sample mix up
```{bash}
#!/bin/bash
...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515/PGCED_SE_ANGI_Martin_v1_3_190515
remove=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515/remove.txt
...
png("Martin_het_F.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515/PGCED_SE_ANGI_Martin_v1_3_190515
path2out=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515
plink --bfile ${DATA}_flipped \
          --freq \
          --out ${path2out}/freq_Martin
```

## DELTA
### Copy AF of data across to Delta
```{bash}
#!/bin/bash
# 
#PBS -S /bin/bash
...
```{bash}
library(data.table)
dat = data.frame(fread("/scratch/90days/uqawal15/1000G_phase3/Martin/freq_Martin.frq", header = T, showProgress = F))
...
refout='/scratch/90days/uqawal15/1000G_phase3/Martin/1000G_phase3_20130502_combined_snpsonly'
...
#SBATCH --time=0:30:00
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515/PGCED_SE_ANGI_Martin_v1_3_190515
pcref='/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515/1000G_phase3_20130502_combined_snpsonly'
...
Rscript  /nfs/home/aliwal/software/PCA_ancestrycalling_meanSD.R  MARTIN    ${pcref}.05.non.outliers.pca3   ${DATA}_cleaned.pca3.proj.eigenvec    ${STR}

```

# Table of Samples in each Ancestry
```{r}
anc <- read.table("MARTIN_ancestry_assigned_with_meanSD_method.txt", header =T)
```

  AMR   EAS   EUR other   SAS
   37    34  1943    17     7
...
```{bash}
cat MARTIN_ancestry_assigned_with_meanSD_method.txt | sed -e '1d' | awk -v OFS='\t' '{if ($7 == "EUR") print $1, $2}' > EUR_samples.txt
```
...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515/PGCED_SE_ANGI_Martin_v1_3_190515
eur=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515/PCA/EUR_samples.txt
...
```{r}
prefix="PGCED_SE_ANGI_Martin_v1_3_190515_EUR"
...
png("Martin_EUR_GRM.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
hist(GRM$off, breaks = seq(-0.2, 1, 0.05), labels = T, main="Distribution of GRM lower triangle", xlab="relationship", cex.main=2, cex.lab=2, cex.axis=2)
dev.off()

off <- GRM$off
off2 <- off[off<0.05]
png("Martin_EUR_GRM_rm05.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
...
off3 <- off[off>=0.05]
png("Martin_EUR_GRM_greater_05.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
...
anc <- read.table(paste0("/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Martin_v1_3_190515", "/PCA/MARTIN_ancestry_assigned_with_meanSD_method.txt"), header = T)
anc <- anc %>% select(FID, population)

final <- merge(het, anc, by = "FID")

fam_05 <- read.table("PGCED_SE_ANGI_Martin_v1_3_190515_EUR_relatedness_005.family.txt", header = F)
related_05 <- unique(fam_05$V1, fam_05$V3)
fam_2 <- read.table("PGCED_SE_ANGI_Martin_v1_3_190515_EUR_relatedness_02.family.txt", header = F)
...
mv PGCED_SE_ANGI_Martin_v1_3_190515_final* final
mv PGCED_SE_ANGI_Martin_v1_3_190515_noHH.hh final
```
