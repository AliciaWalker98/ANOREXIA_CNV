...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Kaye_190430/PGCED_SE_ANGI_Kaye_190430
...
png("Kaye_Summary.png", width=18, height=18, units='in', bg="white", res=150, type=c("cairo"))
par(mfrow=c(2,2))
...
--geno: 8669 variants removed due to missing genotype data.
...
--hwe: 199 variants removed due to Hardy-Weinberg exact test (founders only).
...
117012 variants removed due to allele frequency threshold(s)
...
```{r}
#kaye <- read.csv("/nfs/home/aliwal/ANGI/data/sample_sheets/angi_kaye_190429_sample.tsv", sep = "\t")
#kaye <- kaye[,c("chip_well_barcode", "reported_gender")]


fam <- read.table("/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Kaye_190430/PGCED_SE_ANGI_Kaye_190430_cleaned4.fam", col.names = c("FID", "IID", "X", "X2", "Sex", "Phenotype"))
#fam$Sex <- kaye$reported_gender[match(fam$IID, kaye$chip_well_barcode)]
#fam$Sex <- as.vector(ifelse(fam$Sex == "F", 2, ifelse(fam$Sex == "M", 1, 0)))
fam$Sex <- 2
write.table(fam, "/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Kaye_190430/PGCED_SE_ANGI_Kaye_190430_cleaned_wsex.fam", col.names = F, row.names = F, sep = "\t", quote=F)
```


```{bash}
cp PGCED_SE_ANGI_Kaye_190430_cleaned4.bed PGCED_SE_ANGI_Kaye_190430_cleaned_wsex.bed
cp PGCED_SE_ANGI_Kaye_190430_cleaned4.bim PGCED_SE_ANGI_Kaye_190430_cleaned_wsex.bim
```
...
grep -v OK SexCheck.sexcheck 
  ...
  201145010044_R10C02   201145010044_R10C02            2            0      PROBLEM       0.5145


# Remove the sample mix up
remove.txt

201145010044_R10C02   201145010044_R10C02
```{bash}
#!/bin/bash
...
png("Kaye_het_F.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Kaye_190430/PGCED_SE_ANGI_Kaye_190430
path2out=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Kaye_190430
plink --bfile ${DATA}_flipped \
          --freq \
          --out ${path2out}/freq_Kaye
```

```{bash}
library(data.table)
dat = data.frame(fread("/scratch/90days/uqawal15/1000G_phase3/Kaye/freq_Kaye.frq", header = T, showProgress = F))
...
## plot
library(ggplot2)
png("Kaye_1KG_AF.png", width=15, height=15, units='in', bg="white", res=150, type=c("cairo"))
...
refout='/scratch/90days/uqawal15/1000G_phase3/Kaye/1000G_phase3_20130502_combined_snpsonly'
...
#SBATCH --time=0:30:00
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Kaye_190430/PGCED_SE_ANGI_Kaye_190430
pcref='/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Kaye_190430/1000G_phase3_20130502_combined_snpsonly'
...
Rscript  /nfs/home/aliwal/software/PCA_ancestrycalling_meanSD.R  KAYE    ${pcref}.05.non.outliers.pca3   ${DATA}_cleaned.pca3.proj.eigenvec    ${STR}

```

# Table of Samples in each Ancestry
```{r}
anc <- read.table("KAYE_ancestry_assigned_with_meanSD_method.txt", header =T)
```
...
```{bash}
cat KAYE_ancestry_assigned_with_meanSD_method.txt | sed -e '1d' | awk -v OFS='\t' '{if ($7 == "EUR") print $1, $2}' > EUR_samples.txt
```
...
DATA=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Kaye_190430/PGCED_SE_ANGI_Kaye_190430
eur=/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Kaye_190430/PCA/EUR_samples.txt
...
```{r}
prefix="PGCED_SE_ANGI_Kaye_190430_EUR"
...
png("Kaye_EUR_GRM.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
hist(GRM$off, breaks = seq(-0.1, 0.6, 0.05), labels = T, main="Distribution of GRM lower triangle", xlab="relationship", cex.main=2, cex.lab=2, cex.axis=2)
dev.off()

off <- GRM$off
off2 <- off[off<0.05]
png("Kaye_EUR_GRM_rm05.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
...
off3 <- off[off>=0.05]
png("Kaye_EUR_GRM_greater_05.png", width=15, height=18, units='in', bg="white", res=150, type=c("cairo"))
hist(off3, labels = T,  breaks = seq(0.05, 0.6, 0.05), main="Distribution of GRM lower triangle >= 0.05", xlab="relationship", cex.main=2, cex.lab=2, cex.axis=2)
dev.off()
```
...
anc <- read.table(paste0("/nfs/home/aliwal/ANGI/data/PLINK_GENO/PGCED_SE_ANGI_Kaye_190430", "/PCA/KAYE_ancestry_assigned_with_meanSD_method.txt"), header = T)
anc <- anc %>% select(FID, population)

final <- merge(het, anc, by = "FID")

fam_05 <- read.table("PGCED_SE_ANGI_Kaye_190430_EUR_relatedness_005.family.txt", header = F)
related_05 <- unique(fam_05$V1, fam_05$V3)
fam_2 <- read.table("PGCED_SE_ANGI_Kaye_190430_EUR_relatedness_02.family.txt", header = F)
...
mv PGCED_SE_ANGI_Kaye_190430_final* final
mv PGCED_SE_ANGI_Kaye_190430_noHH.hh final
```
